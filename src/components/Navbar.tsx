...
export function Navbar() {
  ...
  return (
    <nav className="bg-black text-surface-white sticky top-0 z-50 w-full border-b border-white/10">
      {/* Top Row */}
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex justify-between items-center h-16 border-b border-white/10">
        <Link
          to="/"
          onClick={closeMenu}
        ...
        <div className="hidden lg:flex items-center gap-4">
          {TOP_NAV_ITEMS.map((item) => (
             <Link key={item.name} to={item.path} className="text-white/60 hover:text-white text-sm transition-colors">
                {item.name}
             </Link>
          ))}
           ...
           <Link to="/mp-login" className="px-3 py-1.5 bg-white/10 hover:bg-white/20 text-white rounded text-sm transition-colors font-medium">
             MP Login
           </Link>
        </div>
      ...
      {/* Bottom Row */}
      <div className="hidden lg:flex max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-12 flex-row flex-wrap items-center justify-start gap-1 xl:gap-3">
        {NAV_ITEMS.map((item) => {
            const isActive =
        ...
        <div className="lg:hidden absolute top-full left-0 w-full h-[calc(100vh-64px)] overflow-y-auto bg-black border-b border-white/10 shadow-sm animate-in slide-in-from-top-2">
          <div className="px-4 py-4 space-y-1">
            ...
            <div className="pt-4 mt-4 border-t border-white/10 space-y-1">
               {TOP_NAV_ITEMS.map((item) => (
                 <Link key={item.name} to={item.path} onClick={closeMenu} className="block text-base font-semibold text-white/90 hover:text-white py-3">
                   {item.name}
                 </Link>
               ))}
               <Link to="/mp-login" onClick={closeMenu} className="block text-base font-semibold text-white/90 hover:text-white py-3">
                 MP Login
               </Link>
